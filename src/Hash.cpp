...
unsigned int Hash::LeftRotate(unsigned int x, int c)
{
    return (x << c) | (x >> (32 - c));
}
...
std::string Hash::ConvertMD5ToString(const MD5Hash& md5Hash)
{
    ...
    return StringUtility::ToUpper(stream.str());
}

unsigned long long Hash::GetMD5(const std::string& message)
{
    const MD5Hash md5Hash = MD5(message);
    const uint32_t high = ((md5Hash.a >> 24) & 0x000000FF) | ((md5Hash.a >> 8) & 0x0000FF00) | ((md5Hash.a << 8) & 0x00FF0000);
    const uint32_t low = ((md5Hash.b >> 24) & 0x000000FF) | ((md5Hash.b >> 8) & 0x0000FF00) | ((md5Hash.b << 8) & 0x00FF0000) | ((md5Hash.b << 24) & 0xFF000000);
